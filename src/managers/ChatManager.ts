...
class ChatManager{
    ...
    constructor(){
        ...
        GD.S_WS_AUTHORIZED.add(async ()=>{
            ...
            if(this.sharedObject?.messenger)
                return;
        })
        ...
        GD.S_CHAT_MESSAGE_SEND_REQUEST.add(async txt=>{
            ...
            // 4. send epStarted
            if(this.chat.pointID && this.chat.pointID>0)
                this.sendEntryPointStart(msg)
        })
        ...
        GD.S_IMAGE_REQUEST.add(async req=>{
            ...
            if(img)
                GD.S_IMAGE_READY.invoke({uid:req.uid,b64:img,thumb:req.thumb});
        })
    ...
    private onFileUploaded(tempUID:string,error:boolean){
        ...
        setTimeout(()=>{
            ...
            for(let i of this.uploadingFiles){
                if(i[1].status!=="start")
                    this.uploadingFiles.delete(i[1].tempUID)
            }
            this.fireFiles();
        },1000*3)
        this.fireFiles();
    ...
    private isImage(file:File):Boolean{
        ...
        return file.type.indexOf("jpeg")!==-1 ||
