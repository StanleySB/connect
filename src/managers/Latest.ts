...
export default class Latest {
  ...
  constructor() {
    ...
    GD.S_AUTH_COMPLETE.add(async (data) => {
      ...
      if (this.sharedObject && this.sharedObject.entryPointId) epID = this.sharedObject.entryPointId;

      if (this.sharedObject && this.sharedObject.promptMessage) promptMessage = this.sharedObject.promptMessage;
      ...
      if (this.sharedObject && this.sharedObject.title) chat.title = this.sharedObject.title;
    ...
    GD.S_LATEST_REQUEST.add((req) => {
      if (req.viaServer) this.loadChats();
      else this.fireLatest();
    });

    GD.S_CHAT_OPENED.add((cvo) => {
      if (this.sharedObject && !this.sharedObject.messenger) return;
    ...
    GD.S_CHAT_OPEN_REQUEST.add((req) => {
      ...
      if (req.userUID) {
        this.openChatByUserUID(req.userUID);
      }
    });
    ...
    GD.S_MESSAGE_ADDED.add(async (msg) => {
      ...
      if (!cvo) return;
      if (msg.num === cvo.message?.num && msg.text === cvo.message?.text) return;
      if (msg.num < cvo.message?.num) return;
      cvo.message = msg;
      this.setupChatTime(cvo);

      const curr = await GD.REQ_CURRENT_CHAT.invoke();
      if (curr && curr.uid === msg.chatUID) return; // do not fire when msg from opened chat
  ...
  private saveLastReadMsg() {
    ...
    setTimeout(() => {
      ...
      for (let i of this.lastReadMsgNum) {
        if (i[0]) arr.push({ uid: i[0], num: i[1] });
      }
  ...
  private async getChatByUserUID(usersUid: Array<string>) {
    let cvo = null;
  ...
  private async openChatByUserUID(usersUid: Array<string>) {
    const cvo = await this.getChatByUserUID(usersUid);
    if (!cvo) {
      //TODO: oad chat from server
      GD.S_ALERT_ERROR.invoke("Can't open chat, no chat found");
      return;
  ...
  private async loadChats() {
    ...
    if (this.loadLocalData) {
      ...
      if (lastReadStr) {
        try {
          ...
          for (let i of lastRead) {
            if (i.uid) this.lastReadMsgNum.set(i.uid, i.num);
          }
        } catch (e) {
      ...
      if (latestData) {
        let chats;
        try {
          chats = JSON.parse(latestData);
        } catch (e) {}
        if (chats) {
          for (let i of chats) await this.addChat(i, false);
          this.fireLatest();
        }
      }
    ...
    if (resp.data && "hash" in (resp.data as SimpleObjectVO)) localStorage.setItem("dcc_live_latest_hash", (resp.data as SimpleObjectVO).hash);
    ...
    if (resp.data && "latest" in (resp.data as SimpleObjectVO) && Array.isArray((resp.data as SimpleObjectVO).latest)) {
      ...
      for (let i in latest) await this.addChat(latest[i], false);
  ...
  private async addChat(cvo: any, fireSignal = true, parseMessage: boolean = true) {
    ...
    } else this.chats.push(chatVO);

    if (this.chats.length > 500) this.chats.shift();
    if (fireSignal) this.fireLatest();
    return chatVO;
  }

  private async prepareChat(cvo: any, parseMessage: Boolean): Promise<ChatVO> {
    if (cvo.parsed) return cvo;

    if (cvo.securityKey === "company") cvo.securityKey = Services.companySequence;
    let title = cvo.title;

    if (title.startsWith("!")) cvo.title = Services.unpack(title.substr(1), cvo.securityKey);
    else {
      if (cvo.users) {
        title = "";
        for (let usr of cvo.users) {
          if (usr.uid === this.profile?.uid) continue;
          if (title !== "") title += ", ";
          title += usr.name;
        }
        if (title !== "") cvo.title = title;
      }
    }
  ...
  private setupChatTime(cvo: ChatVO) {
    ...
    if (cvo.message && cvo.message.user_uid === this.profile?.uid) cvo.author = "you";
    if (cvo.type === "private" && cvo.message && cvo.message.user_uid !== this.profile?.uid) cvo.author = null;
  }

  private async fireLatest() {
    this.chats.sort((a, b) => {
      if (a.chatTime > b.chatTime) return -1;

      if (b.chatTime > a.chatTime) return 1;
    ...
    for (let i of this.chats) {
      ...
      if (prev && i.DOY === prev.DOY) i.position = "middle";
      prev = i;
      if (this.currentChat && i.uid === this.currentChat.uid) i.selected = true;

      let lastRead = this.lastReadMsgNum.get(i.uid);
      if (!lastRead || lastRead < 1) lastRead = -1;
      ...
      if (i.message && i.message.user_uid === this.profile?.uid) i.unread = 0;
      if (i.unread < 0) i.unread = 0;
    }
